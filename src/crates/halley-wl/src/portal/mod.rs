...
fn resolve_cursor_meta_for_region(
    ...
    // Static cursor snapshot for screencast metadata: we use the first
    // frame (no animation here — the client receives a static image).
    Some(CursorMetaFrame {
        ...
        bgra: sprite.first_frame().pixels_bgra.clone(),
    })
}
