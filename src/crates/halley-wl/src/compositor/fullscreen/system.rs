...
/// True when the current render monitor has a settled fullscreen window.
///
/// Used by the frame builder to suppress the Top/Overlay layer-shell surfaces
/// (the bar, notifications, launcher) so a fullscreen app covers them like
/// every other WM does. Gated on the visual animation being idle so the bar
/// re-appears while the fullscreen zooms out on exit instead of popping back
/// only at the end.
...
fn request_toplevel_fullscreen_state(
    ...
    for top in st.platform.xdg_shell_state.toplevel_surfaces() {
        ...
        // Dedup: emit a configure ONLY when the pending state differs from the
        // last sent one. A client that is already fullscreen and requests
        // `set_fullscreen` again (Chromium/Electron do this on tab/video
        // switches; see the `ClientRequest` early-return in
        // `enter_xdg_fullscreen_inner`) used to receive an IDENTICAL configure
        // with Fullscreen reasserted. The browser reads that as "you just
        // entered fullscreen" and re-shows the enter-fullscreen toast, which for
        // that reason never dismisses.
        //
        // `send_pending_configure()` is Smithay's own dedup: a no-op when the
        // pending state equals the already-sent one. Same pattern already used
        // in `compositor/overlap/system/resolve.rs`, where the unconditional
        // configure flooded the client on every maintenance tick.
