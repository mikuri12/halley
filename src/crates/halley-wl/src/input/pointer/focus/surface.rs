...
pub(crate) fn layer_surface_focus_for_screen(
    ...
    // A settled fullscreen COVERS the Top/Overlay layers: the frame builder
    // skips drawing them (`suppress_top_overlay_layers` in render/frame/scene.rs).
    // If the hit-test doesn't skip them too, the bar, notifications and the
    // click-shield stay INVISIBLE BUT CLICKABLE: the user clicks "inside" the
    // fullscreen app and the click is swallowed by a layer they can't see.
    //
    // We use the SAME predicate as the render path
    // (`current_monitor_has_settled_fullscreen`) and NOT `block_non_overlay`,
    // because the latter only triggers when the pointer HITS the fullscreen
    // node (`pick_hit_node_at`): over an area covered by a suppressed layer
    // that hit-test fails, so the block didn't engage exactly where it's needed.
    ...
    // mapped *before* a panel on the same layer (e.g. a click-shield) wins the
    // hit test over the panel above it, so clicks inside the panel are delivered
    // to the shield and dismiss the panel.
    placements.reverse();
    ...
    for placement in placements {
        // Layers suppressed by the render path: also invisible to the hit-test.
