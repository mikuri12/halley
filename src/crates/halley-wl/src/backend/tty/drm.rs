...
pub(crate) fn queue_tty_drm_frame(
    ...
    let result = (|| {
        ...
        // The direct-scanout path is already paced by page-flip + presentation
        // feedback (further below), so it must NOT be disabled just because
        // frame-callbacks are pending. A fullscreen client ALWAYS has pending
...
fn direct_scanout_cursor_elements(
    ...
    match cursor_status {
        ...
        CursorImageStatus::Named(icon) => {
            // Sprite via the CursorManager (NOT the global cache): this keeps
            // the animation state (started_at/cycle_ms) authoritative also when
            // the cursor goes out via the DRM HW plane, where draw_cursor_layer
            // does not run.
...
fn fullscreen_direct_scanout_candidate(
    ...
    // Top/Overlay layer-shell surfaces (bar, notifications, launcher) are
    // SUPPRESSED on a settled fullscreen via `suppress_top_overlay_layers`.
    // If they still blocked direct-scanout here, the fullscreen app would fall
    // back to the composited (GL) path with the layers equally invisible =
    // slow for no reason. We already passed the "animation in progress" gate
    // above, so when this candidate is the active fullscreen node on the
    // monitor it is settled -> layers suppressed.
