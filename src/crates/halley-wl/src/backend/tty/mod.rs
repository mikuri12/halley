...
/// Master timer period when there is no pending work. Rendering and input
/// arrive by event; this tick only covers the lazy paths (child reap, IPC,
/// config watch, stuck-frame timeout).
    ...
    scope!(
        ...
        aborted = "compositor aborted",
        {
            ...
            ev.handle().insert_source(timer, move |_tick, _, st| {
                ...
                // Adaptive tick: at 60 Hz this timer was the heartbeat that
                // drove the whole compositor, and it re-armed on every iteration
                // (idle CPUs stayed busy). Real work now arrives by event
                ...
                let busy = !st.runtime.tty_redraw_outputs.is_empty()
                    ...
                    || output_frame_pending_for_dpms_timer
                        ...
                        .values()
                        .any(|pending| *pending)
                    || pending_watch_reload_at_for_timer.borrow().is_some()
                    || pending_output_rescan_at_for_timer.borrow().is_some()
                    || secs < 6
                    || tty_animation_redraw_active(
                        st,
                        &outputs_for_timer,
                        &pointer_state_for_timer,
                        now,
            ...
            // Register the display fd in calloop so a client commit wakes
            // the loop on its own. Without this, the only thing that serviced
            // clients was the 16.7 ms timer, leaving the compositor polling
            // permanently at idle.
            ev.handle().insert_source(
                ...
                |_readiness, display, st| {
                    // SAFETY: el display no se suelta mientras la fuente vive.
                    unsafe {
                        display.get_mut().dispatch_clients(st)?;
                    }
                    Ok(calloop::PostAction::Continue)
                },
            )?;

            let mut dh_for_flush = dh.clone();
