...
pub(crate) fn tty_output_animation_redraw_state(
    ...
    // Animated themed cursor: force an output redraw so the cursor blit
    // advances its frame by elapsed time (there is no dedicated timer). This
    // reuses the same mechanism as the animated background. The sprite cache
